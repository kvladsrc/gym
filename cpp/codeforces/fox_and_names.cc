...
int main(int /*argc*/, char * /*argv*/[]) {
  ...
  while (true) {
    ...
    for (size_t char_a = 0; char_a < 26; ++char_a) {
      for (size_t b = char_a + 1; b < 26; ++b) {
        auto ba = reachable(res[b], res[char_a], g);
        auto ab = reachable(res[char_a], res[b], g);
        if (ab && ba) {
          err = true;
          break;
        }
