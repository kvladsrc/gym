...
int main(int argc, char* argv[]) {
  ...
  while (!all.empty()) {
    ...
    while (!stack.empty()) {
      ...
      std::transform(points_by_x[p.first].begin(), points_by_x[p.first].end(),
                     std::back_inserter(stack),
                     [&p](int y) { return std::make_pair(p.first, y); });
      std::transform(points_by_y[p.second].begin(), points_by_y[p.second].end(),
                     std::back_inserter(stack),
                     [&p](int x) { return std::make_pair(x, p.second); });
    }
