...
Drop the strips into `games/cothic/tmp/sprites/knight/<animation>/strips/` as
...
atlas to `games/cothic/tmp/sprites/knight/walk.png`.
Useful flags (passed after the
animation name):
...
`games/cothic/tmp/sprites/`, which is intentionally ignored by Git. Approved final
